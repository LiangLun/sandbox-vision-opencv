...
void Train() {
    ...
    ifstream trainingData(string(FILEPATH) + "svmTrainSet.txt");
    unsigned long n;
    ...
    cout << "hog compute:" << endl;

    // 计算HOG特征
    for (vector<string>::size_type i = 0; i <= imagePath.size() - 1; ++i) {
        //读入图片
        string image_path = FILEPATH + imagePath[i];
        Mat src = imread(image_path, -1);
        if (src.empty()) {
            cout << "can not load the image:" << image_path << endl;
            continue;
        }
    ...
    for (;;) {
        ...
        if (f) {
            ...
            while (l > 0 && isspace(filename[l - 1]))
                --l;
            filename[l] = '\0';
            img = imread(string(FILEPATH) + filename);
        }
        printf("%s:\n", filename);
        if (!img.data)
            continue;
...
void HOG_SVM1() {
    ...
    int numberOfLine = 0;
    string buffer;
    ifstream trainingData(string(FILEPATH) + "svmTrainSet.txt", ios::in);
    unsigned long n;
