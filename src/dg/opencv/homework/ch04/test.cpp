/*
 *
 *  1. 对raccoon_hw.jpg图片中，两只浣熊的部分分割出来，上传提交分割后的二值图。
 *     TIPS：先使用灰度分割到二值图，然后通过findContours找到所有轮廓，然后利用大小、形状，把干扰的轮廓去除掉。
...
int main(int argc,char* argv[]){
	...
	cv::Mat show_image(cv::Size(width,height),CV_8UC3);
	...
	// 矩形框
	//cv::rectangle(image,cv::Point(20,20),cv::Point(50,50),cv::Scalar(20,0,0));
	// 附加文字
	//putText( image, "dataguru", cv::Point( image.rows/2,image.cols/4),CV_FONT_HERSHEY_COMPLEX, 1, cv::Scalar(30, 0, 0) );
	// 拷贝到显示图片指定位置（下方）
